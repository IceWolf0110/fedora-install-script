...
grep -qxF "$line1" /etc/dnf/dnf.conf || echo "$line1" >> /etc/dnf/dnf.conf
grep -qxF "$line2" /etc/dnf/dnf.conf || echo "$line2" >> /etc/dnf/dnf.conf
...
sudo dnf install gnome-shell file-roller firefox unzip unrar p7zip gnome-backgrounds nautilus gnome-terminal gnome-terminal-nautilus gnome-software gnome-control-center gnome-system-monitor
...
echo "installed Extension Manager" 
